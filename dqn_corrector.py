...
class TDReplayBuffer:
    def __init__(self):
        ...
        obs_dim = config.sample_size * config.hidden_dim
...
class DQNCorrector:
    def __init__(self, agent, buffer, classifier, logger):
        self.agent = agent
        self.buffer = buffer
    ...
    def sample(self, num):
        candidate = np.random.choice([i for i in range(len(self.dataset_u))], size=num,
                                     replace=False)
