...
class DQNAgent(nn.Module):
    def __init__(self):
        ...
        self.obs_dim = config.sample_size * config.hidden_dim
        ...
        self.q_optimizer = torch.optim.SGD(self.q_network.parameters(), lr=config.rl_learning_rate)
    ...
    def try_update_target_network(self, tau):
        if self.tot_num_updates % self.update_target_network_interval == 0:
            soft_update_network(self.q_network, self.q_target_network, tau)

    def select_center(self, obs):
        ...
        center = action_indices.detach().cpu().numpy()[0]
        return center
